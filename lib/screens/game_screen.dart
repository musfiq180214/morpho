...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  ...
  void _spawnInitialWalls() {
    for (int i = 0; i < 4; i++) {
      _walls.add(Wall.generate('w${_wallCounter++}', 1.0 + i * _wallSpacing));
    }
  }
  ...
  void _startGameLoop() {
    _gameTimer =
        Timer.periodic(const Duration(milliseconds: 1000 ~/ 60), (timer) {
          ...
          if (gs.isPaused || gs.phase == GamePhase.dead ||
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // 1. Vertical Drag Layer (Background + Game)
          GestureDetector(
            behavior: HitTestBehavior.opaque,
            // Ensures it catches drags anywhere
            onVerticalDragUpdate: (details) {
              if (gs.isPaused || gs.phase == GamePhase.dead) return;
              setState(() {
                double delta = details.primaryDelta! / size.height;
                _blobY = (_blobY + delta).clamp(0.1, 0.85);
              });
            },
            child: Stack(
              children: [
          ...
          // 3. Control Layer (Important: This is ABOVE the GestureDetector)
  ...
  Widget _buildBackground(Size size) {
    return AnimatedBuilder(
      ...
      builder: (_, __) =>
          CustomPaint(
              size: size, painter: _BackgroundPainter(_bgController.value)),
    );
  }
  ...
  Widget _buildGameCanvas(Size size, GameState gs, Color shapeColor) {
    return AnimatedBuilder(
      animation: Listenable.merge(
          [_pulseController, _wobbleController, _wallAnimController]),
      builder: (_, __) =>
          CustomPaint(
            size: size,
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: ShapeType.values.map((shape) {
            return ShapeButton(
              shape: shape,
              isActive: gs.currentShape == shape,
...
class _GameCanvasPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final wall in walls) {
      WallPainter(
        wall: wall,
        wallColor: const Color(0xFF1A1A3A),
        holeColor: const Color(0xFF0A0A1A),
        glowColor: shapeColor,
        animValue: wallAnim,
      ).paint(canvas, size);
    }
    ...
    final blobOffset = Offset(
        size.width * 0.25 - blobSize / 2, blobY * size.height - blobSize / 2);
    canvas.save();
...
class WallPainter {
  ...
  WallPainter(
      {required this.wall, required this.wallColor, required this.holeColor, required this.glowColor, required this.animValue});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final holeRect = Rect.fromCenter(
        center: Offset(xPos + wallWidth / 2, holeY),
        width: holeSize,
        height: holeSize);
    ...
    canvas.drawPath(
        Path.combine(PathOperation.difference, wallPath, holePath), Paint()
      ..color = wallColor);
...
class _BackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = -1; i <= 16; i++)
      canvas.drawLine(
          Offset(0, i * rowH + offset), Offset(size.width, i * rowH + offset),
          p);
  }
...
class _Particle {
  ...
  _Particle(
      {required this.x, required this.y, required this.vx, required this.vy, required this.life, required this.size, required this.color});
}